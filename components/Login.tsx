...
interface LoginProps {
  onLoginSuccess: (name: string, phone: string, esAdmin?: boolean) => void;
...
export default function Login({ onLoginSuccess, onSwitchToRegister }: LoginProps) {
  ...
  async function handleSendCode(e: React.FormEvent) {
    ...
    try {
      ...
      setStep(2);
      // We set a simulated state to show the hint box
  ...
  async function handleVerifyCode(e: React.FormEvent) {
    ...
    try {
      ...
      onLoginSuccess(data.user.nombre, phone, data.user.es_admin);
    } catch (err: any) {
      setError(err.message || "Error de conexión.");
    } finally {
      setIsLoading(false);
    ...
    <div className="w-full max-w-xl mx-auto glass rounded-3xl p-6 sm:p-8 shadow-xl shadow-primary/5 dark:shadow-black/40 border border-border-subtle animate-slide-up">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm sm:text-base text-text-muted">
          Accede de forma rápida y segura sin contraseñas
        </p>
      </div>
        ...
        <form onSubmit={handleSendCode} className="space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-text-muted leading-relaxed">
              Ingresa los 10 dígitos de tu número celular. Te enviaremos un código SMS gratuito para verificar tu identidad.
            </p>
